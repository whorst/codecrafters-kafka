...
func (c *ClusterMetadata) ParseClusterMetadataFileByTopicNames(topicNames []string) (error, clutser_metadata_port.ClusterMetadataLogResponse) {
	...
	c.ClusterMetadataLogResponse.TopicUUIDPartitionMetadataMap = make(map[string][]*clutser_metadata_port.PartitionMetadata)
...
func (c *ClusterMetadata) processTopicRecord(data []byte, offset int) {
	...
	tmi := clutser_metadata_port.TopicMetadataInfo{
		...
		PartitionsArray:           []*clutser_metadata_port.PartitionMetadata{},
		TopicAuthorizedOperations: []byte{0x00, 0x00, 0x00, 0x00},
		TagBuffer:                 []byte{0x00},
	}

	c.ClusterMetadataLogResponse.TopicUUIDTopicMetadataInfoMap[string(topicUuid)] = &tmi
	_ = version
}
...
func (c *ClusterMetadata) processPartitionRecord(data []byte, offset int) {
	...
	partitionRecord := clutser_metadata_port.PartitionMetadata{}
	c.TopicUUIDPartitionMetadataMap[string(topicUuid)] = append(c.TopicUUIDPartitionMetadataMap[string(topicUuid)], &partitionRecord)
	...
	topicPartitionMetadataSlice := c.TopicUUIDPartitionMetadataMap[string(topicUuid)]
